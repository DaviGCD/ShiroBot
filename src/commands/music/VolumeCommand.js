...
module.exports = class VolumeCommand extends Command {
    ...
    run(ctx) {
        ...
        if (!role) {
            ...
            if (ctx.args[0] > 100 && ctx.args[0] < 5) return ctx.quote(ctx.locale('commands:volume.max-and-min', { volume: ctx.client.player.get(ctx.message.guildID).player.state.volume }))
